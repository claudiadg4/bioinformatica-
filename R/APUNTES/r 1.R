...
# Para asignar valores se puede usar indistintamente "<-" o "=" #es lo mismo la fecha y el igual

# Numeric: contienen números con decimales
# Booleanos: TRUE or FALSE
# Strings: secuencias de caracteres
#en R sobretodo tenemos listas y tablas, no diccionarios

# Nombres de variables: pueden incluir puntos (en phyton no se podia), NO PONER GUIONES EN VEZ DE ESO BARRA BAJA"_" pero NUNCA "-". NUNCA empiezan por números. NO GUIONES
...
# Series (listas o vectores)#vectores tienen un factor matematico (numeros), las listas pueden ser palabras
...
rep(1,10)#te repite el numero 1 diez veces

seq(2,6)#OJO EN R SE CUANTA DESDE EL 1 NO DESDE EL 0. #DEL 2 AL 6

seq(4,20,by=4)#HASTA EL 20 DE CUATRO EN CUATRO

#Para navegar por ellos, se buscan sus posiciones
x[1]
x[-1]#TODOS MENOS EL 1 (EN PHYTON ERA EL ULTIMO)
x[1]<-3;x  # el ";" permite poner dos líneas de código en una sola, una detrás de la otra#HEMOS CAMBIADO NUMERO DE LA POSICION 1 POR EL 3
x[-1]<-5;x  # reasignamos valores dentro del vector

y<9 # nos devuelve un booleano dependiendo de si los elementos de ese vector cumplen ese criterio#TODOS LOS NUMEROS DEL VECTOR MENORES A NUEVE SE CAMBIAN A 2
...
bp[,2] 		# Lo mismo pero no aparece el nombre de la columna #dolar es lo mismo que corchete
bp$WEIGHT 		# lo mismo
...
head(iris)#si pones el head es el encabezado    # dataset integrado en R
str(iris)
summary(iris)#caracteristicas de la tabla, min, max, mediana, q1,q2...
...
##### 7.- VECTORES / LISTAS AVANZADOS# es decri listas dentro de listas
...
genesets[[1]]
genesets[[1]][1]
unlist(genesets[1])[1]#este no importante
length(genesets)#cuidado con el nombre que no es el mimso que phyton
length(genesets[[2]])
...
for(j in 1:3){
lista_vacia[[j]] <- c(1,2,3*j)#la j es un indicador de posicion en un bucle
}
lista_vacia
